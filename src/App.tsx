...
// Canonical multi-tenant pattern (per WorkOS docs):
//   - VITE_WORKOS_CLIENT_ID  — shared MyJarvis AuthKit client
//   - VITE_WORKOS_ORG_ID     — THIS tenant's WorkOS Organization ID
// signIn() passes `organizationId` so AuthKit issues a token scoped to this org.
// WorkOS itself rejects sign-in if the user isn't a member of the org —
// access control is enforced at the IdP, not in our app.
...
if (!workosOrgId) {
  console.warn(
    "[dashboard] VITE_WORKOS_ORG_ID is not set — sign-in will not be scoped to a tenant org",
  );
}
...
function AuthGate() {
  ...
  // User is signed in. Per WorkOS canonical pattern, the token already carries
  // an `org_id` claim equal to `workosOrgId` (or WorkOS would have refused to
  // issue it). Server-side `requireUser` re-verifies the claim. No custom
  // tenant access endpoint required.
  return (
    <HashRouter>
